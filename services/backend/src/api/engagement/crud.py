...
from src.api.engagement.models import (
    Engagement, EngagementType, LikeDislike
)
...
def get_like_count_by_content_id(content_id):
    return _get_engagements_query_by_content_id(content_id, EngagementType.Like).filter_by(engagement_value=int(LikeDislike.Like)).with_entities(func.count()).scalar()

def get_dislike_count_by_content_id(content_id):
    return _get_engagements_query_by_content_id(content_id, EngagementType.Like).filter_by(engagement_value=int(LikeDislike.Dislike)).with_entities(func.count()).scalar()
