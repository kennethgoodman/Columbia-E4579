...
class TwoTowerANNGenerator(AbstractGenerator):
    def _get_content_ids(self, user_id, limit, offset, _, starting_point):
        if starting_point.get("content_id", None) is not None:
            content_ids, scores = get_ANN_recommendations_from_content(starting_point["content_id"], "beta", limit + offset)
        else:
            content_ids, scores = get_ANN_recommendations_from_user(user_id, "beta", limit + offset)
