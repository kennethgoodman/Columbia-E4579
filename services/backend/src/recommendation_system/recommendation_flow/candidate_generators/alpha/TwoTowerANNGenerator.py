...
class TwoTowerANNGenerator(AbstractGenerator):
    def _get_content_ids(self, user_id, limit, offset, _, starting_point):
        if starting_point is not None and starting_point.get("content_id", False):
            content_ids, scores = get_ANN_recommendations_from_content(starting_point["content_id"], "alpha", limit + offset)
        else:
            content_ids, scores = get_ANN_recommendations_from_user(user_id, "alpha", limit + offset)
