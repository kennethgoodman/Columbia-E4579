...
def detailed_portrait(prompt):
    return prompt + ". Detailed portrait, detailed face. Award winning. lush detail, wlop, octane render, trending on " \
                    "artstation "


def scifi(prompt):
    return prompt + ". Scifi, in space, very far in the future. Award winning. trending on artstation. CGI. special " \
                    "effects. intense details. trending on artstation "


def studio(prompt):
    return prompt + ". Studio ghibli, lush detail, award winning, wlop, octane render, trending on artstation, " \
                    "by otto dix and greg ruthkowshi "


def oil_on_canvas(prompt):
    return prompt + ". high-quality, Pixiv, WLOP, Greg Rutkowski, ArtStation. Oil on canvas"
...
def medieval(prompt):
    return "Medieval " + prompt + ":: epic landscape, iceland photgraphy, cinematic, octane render, 8k, art station " \
...
def face_and_lighting(prompt):
    return prompt + '. symmetrical face, ambient light, intense detailed. award winning, photo realistic'
...
def should_filter_in(title):
    block_words = ["nsfw", "sex", "fuck", "fucked", "shit", "bitch", "asshole", "ass", "porn", 'niggard', 'shitten']
    title = title.lower()
...
def get_url(subreddit, after, t):
    return f"https://www.reddit.com/r/{subreddit}/top.json?limit=100&after={after}&t={t}"


def get_top_titles_from_subreddit(subreddit, total_limit, t='all'):
    headers = get_headers()
    response = requests.get(f'https://www.reddit.com/r/{subreddit}/top.json?limit=100&t={t}', headers=headers)
    ...
    while len(titles) < total_limit and json_response['data']['after'] is not None:
        response = requests.get(get_url(subreddit, json_response['data']['after'], t), headers=headers)
        json_response = response.json()
        titles |= get_titles(json_response['data']['children'])
        time.sleep(2)
    return titles
...
def get_subreddits():
    return ['pics', 'MadeMeSmile', 'Damnthatsinteresting',
            'AccidentalArt', 'scifi', 'SimplePrompts', 'Showerthoughts',
            'whoahdude', 'oddlysatisfying', 'EarthPorn', 'educationalgifs',
            'RetroFuturism', 'Cyberpunk'
]
...
def transform_raw_prompt_into_row(raw_prompt, source):
    ...
    if len(raw_prompt.split(" ")) > 100:  # more than 100 words, very descriptive already
...
def generate_rows_from_poems():
    ...
    for poem_dict in j:
        ...
        if should_filter_in(poem):
            rows.append(generate_row(edit_prompt(poem), poem, 'NA', f'{poem_dict["title"]} by {poem_dict["author"]}'))
    return rows
