      ...
      new IntersectionObserver(([entry]) => {
            ...
            const handle_elapsed = (elapsed_time) => {
              ...
              fetch(`${api_uri}?content_id=${content_id}`,
                {...postRequestOptions, 'body': JSON.stringify({'elapsed_time': elapsed_time})}
              ).then(response => response.json());
            }
            if(entry.isIntersecting) {
                if(isIntersecting) { // if wasn't currently intersecting, so starting to intersect
                    // already intersecting, do nothing
                    return;
                }
                if(startIntersecting === -1) {
                    console.log(`starting to watch content ${content_id} @ ${Date.now()}`);
                    setStartIntersecting(Date.now());
                }
                handleLoad();
            } else if(startIntersecting === -1){  // first time loading, but not in view

            } else { // not intersecting, and not loaded for first time
                if(startIntersecting !== -1) {
                    console.log(`stopping to watch content ${content_id} @ ${Date.now()}, elapsed: ${Date.now() - startIntersecting}`)
